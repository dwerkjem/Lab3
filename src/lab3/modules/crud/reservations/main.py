...
def _room_choice_label(name: str, day_rate_cents: int) -> str:
    ...
    """
    return f"{name} - ${day_rate_cents / 100:,.2f}"  # Convert cents to dollars for display.
...
def edit_reservations(name_dict: dict[str, int | str]) -> None:
    ...
    if choice == "Make a new reservation":
        ...
        edit_existing_reservation(reservation_list, user_name)
...
def _setup(customer_id: int, room_names: list[str], user_name: str) -> None:
    ...
    Prompts the user for event information, attendees, room, dates, optional
    ...
    if event_name is None or event_type in (None, "Quit") or attendees_count is None:
        sys.exit(0)

    room = get_room(
        user_name,
        minimum_capacity=int(attendees_count),
    )
...
def edit_existing_reservation(
    ...
    room = get_room(
        user_name,
        default=room_name_from_id(room_id),
        minimum_capacity=int(attendees_count),
    )
